...
public class TAlarmPolicyService extends CrudService<TAlarmPolicyDao, TAlarmPolicy> {
    ...
    public JSONObject selectTAlarmPolicys() {

        JSONObject jsonObject = new JSONObject();
        List<MapEntity> selectTIdLinkIds = processDataDao.selectTAlarmPolicys();
        Map map0 = new HashMap();

        Map map = new HashMap();
        for (MapEntity selectTIdLinkId : selectTIdLinkIds) {

            map.put(selectTIdLinkId.get("id").toString(), selectTIdLinkId);
        }
        map0.put("alarm_policy", map);
        jsonObject.put("cmd", "setData");
        jsonObject.put("data", map0);
